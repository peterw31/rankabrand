...
var language = 'en';
var mobileUrl =  'http://m.rankabrand.org/';
function initSearchPage(event) {
	...
	var urlQuery = getParameterByName("q").trim();
	$("#brand-search-input").val(urlQuery);
	var e = $.Event('keyup.brandupdate');
	e.keyCode = 65; // Character 'A'
	$("#brand-search-input").trigger(e);
	...
	chrome.storage.sync.get({
        ...
        function (items) {
            ...
            if (language == 'nl')
            {
                mobileUrl = 'http://m.rankabrand.nl/';
            }
            else if (language == 'de') {
                mobileUrl = 'http://m.rankabrand.de/';
            }
            else
            {
                mobileUrl = 'http://m.rankabrand.org/';
            }
	});
...
function populateBrandsList(response) {
	...
	for (i = 0; i < brandData.length; i++) {
		...
		newBrand.find('.brandpage').attr('href', mobileUrl + 'singlebrand.php?bid=' + brandData[i].id + '&sectorid=' + brandData[i].sectorId + '&active=search');
		newBrand.find('.brandpage').attr('target', '_blank');
		newBrand.find('.brandpage').click(function () { self.close()});
...
function getFromAPI(command, callback) {
	$.get('http://m.rankabrand.nl/rankabrandproxy.php', { command: command, lang: language }).complete(callback);
}
